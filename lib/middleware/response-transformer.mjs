...
/**
 * Filters and transforms data records based on specified column definitions.
 *
 * @param {Object} params - Parameters for updating the data.
 * @param {Array<Object>} params.data - The original array of data records to be updated.
 * @param {Object} params.columns - An object representing column definitions.
 * Each key is a field name, and its value is an object with a `name` property
 * to rename the field in the output.
 *
 * @returns {Array<Object>} - A new array of filtered and transformed data records,
 * keeping only the specified columns and replacing `null` or `undefined` values with an empty string.
 */
const updateData = ({ data, columns }) => {
    if (Object.keys(columns)?.length) {
        const fieldsToKeep = Object.keys(columns); // These are the fields you want to keep
        data = data.map(record => {
            const filteredRecord = {};
            fieldsToKeep.forEach(field => {
                const name = columns[field] ? columns[field].name : field;
                filteredRecord[name] = [null, undefined].includes(record[field]) ? '' : record[field];
            });
...
/**
 * Formats a Date object based on whether it includes time information.
 *
 * @param {Object} params - Parameters for formatting the date.
 * @param {Date|string} params.value - The value to format. If it's a `Date`, it will be formatted using the appropriate format.
 * @param {string} params.dateFormat - The format to use if the date has no time component.
 * @param {string} params.dateTimeFormat - The format to use if the date has a time component.
 * @returns {string|*} - The formatted date string if `value` is a `Date`; otherwise, returns the original `value`.
 */
const formatDate = ({ value, dateFormat, dateTimeFormat }) => {
    if (value instanceof Date) {
        const hasTime = value.getHours() + value.getMinutes() + value.getSeconds() + value.getMilliseconds() > 0;
        return formatDateTime({ value, format: hasTime ? dateTimeFormat : dateFormat });
    } else {
        return value;
    }
}
...
const responseTransformer = async function (req, res, next) {
    res.transform = async function ({ success, ...others }, { responseType, fileName, data: dataField = "data" } = {}) {
        ...
        if (isExportOperation) {
            for (const key in exportColumns) {
                ...
                if (isElastic) {
                    ...
                    columns[key] = { ...commonProperties, name: exportColumn.headerName };
                    columnKeyMappings[exportColumn.field] = exportColumn.headerName;
                }
            }
        ...
        switch (responseType) {
            ...
            case mimeTypes.csv:
                if (data.length > 0) {
                    data = updateData({ data, columns });
                    data = data.map(record => {
                        const formattedRecord = {};
                        for (const [key, value] of Object.entries(record)) {
                            formattedRecord[key] = formatDate({ value, dateTimeFormat , dateFormat: userDateFormat }); // This is a common format for dates in CSV
            ...
            case mimeTypes.xml:
                ...
                const formattedData = data.map(item => {
                    ...
                    for (const key in item) {
                        // Remove all characters except letters, digits, dots, underscores, and hyphens
                        const formattedKey = key.replace(/[^a-zA-Z0-9._-]/g, '');
                        newItem[formattedKey] = formatDate({ value: item[key], dateTimeFormat, dateFormat: userDateFormat }); 
                    }
                    return newItem;
                });
                return res.send(js2xmlparser.parse("root", formattedData));
            case mimeTypes.html:
                if (data.length > 0) {
                    if (Object.keys(columns)?.length === 0) {
                        columns = data.columns || {};
                    ...
                    data = data.map(record => {
                        const formattedRecord = {};
                        for (const [key, value] of Object.entries(record)) {
                            formattedRecord[key] = formatDate({ value, dateFormat: userDateFormat, dateTimeFormat }); // This is a common format for dates in HTML
                        }
                        return formattedRecord;
                    });
