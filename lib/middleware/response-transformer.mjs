...
const xmlArr = [mimeTypes.xlsx, mimeTypes.xml, mimeTypes.html];
...
const responseTransformer = async function (req, res, next) {
    res.transform = async function ({ success, ...others }, { responseType, fileName, data: dataField = "data" } = {}) {
        ...
        let data = null;
        const dataArr = others?.dataField ? others?.dataField : [dataField];
        const exportColumns = others?.exportColumns
        const isExportOperation = (typeof exportColumns === 'object' && Object.keys(exportColumns).length > 0);
        let userDateFormat, isElastic, userTimezoneOffset, dateTimeFormat, lookups, lookupFields, columns = {}, columnKeyMappings = {}, jsonResponse = others;
        if (isExportOperation) {
            userDateFormat = others?.userDateFormat;
            isElastic = others?.isElastic;
            userTimezoneOffset = others?.userTimezoneOffset;
            dateTimeFormat = userDateFormat + util.dateTimeExportFormat;
            lookups = others?.lookups;
            lookupFields = others?.lookupFields;
        ...
        for (const dataFieldKey of dataArr) {
            ...
            if (isExportOperation) {
                ...
                if (responseType === mimeTypes.xlsx) {
                    data = data.data || data;
                }
                if (responseType === mimeTypes.json) {
                    jsonResponse = data;
                }
                if (responseType === mimeTypes.csv || responseType === mimeTypes.html || responseType === mimeTypes.xlsx) {
                    data = ensureArray(data);
                }
                if (exportColumns && Object.keys(exportColumns)?.length > 0) {
                    data = updateKeys({ data, keyMapping: columnKeyMappings, columns, userDateFormat, userDateTimeFormat: dateTimeFormat, userTimezoneOffset, lookups, lookupFields, isForXML: xmlArr.includes(responseType) });
                }
