...
const responseTransformer = async function (req, res, next) {
    res.transform = async function ({ success, ...others }, { responseType, fileName, data: dataField = "data" } = {}) {
        let data = success === true ? others[dataField] : null, isMultiSheet = false;
        if (!success) {
        ...
        if (isExportOperation) {
            ...
            isMultiSheet = Array.isArray(data) && data.every(sheet => sheet.title && sheet.rows && sheet.columns);

            if (!isMultiSheet && exportColumns && Object.keys(exportColumns)?.length > 0) {
                data = updateKeys({ data, keyMapping: columnKeyMappings, columns, userDateFormat, userDateTimeFormat: dateTimeFormat, userTimezoneOffset, lookups, lookupFields, isForXML: isForXMLResponseType.includes(responseType) });
            }
        }
        switch (responseType) {
            case mimeTypes.json:
            ...
            case mimeTypes.xlsx:
                data = data.data || data;
                if (isMultiSheet || data.length > 0) {
                    res.set('Content-Type', responseType);
                    res.set('Content-Disposition', `attachment; filename="${fileName}.xlsx"`);
                    if (isMultiSheet) {
                        return await toExcel({ sheets: data, fileName, stream: res, exportColumns: true, userDateFormat, userDateTimeFormat: dateTimeFormat,  userTimezoneOffset });
                    }
