...
const updateKeys = function ({ data, keyMapping, columns, isForXML = false, userDateFormat, userDateTimeFormat, userTimezoneOffset }) {
        ...
        for (const ele of Object.keys(keyMapping)) {
            ...
            if (util.dateTimeFields.includes(valueType)) {
                if (value !== null && value !== undefined) {
                    value = keepUTC ? dayjs.utc(value) : new Date(value);
                    if (userTimezoneOffset && !keepUTC) {
                        value = new Date(value.getTime() + Number(userTimezoneOffset))
                    }
    ...
    res.transform = async function ({ success, ...others }, { responseType, fileName, data: dataField = "data" } = {}) {
        ...
        switch (responseType) {
            case mimeTypes.csv:
                ...
                if (data.length > 0) {
                    if (exportColumns && Object.keys(exportColumns)?.length > 0) {
                        data = updateKeys({ data, keyMapping: columnKeyMappings, columns, userDateFormat, userDateTimeFormat: userDateFormat + util.dateTimeCSVExportFormat, userTimezoneOffset });
                    }
            ...
            case mimeTypes.json:
                ...
                if (exportColumns && Object.keys(exportColumns)?.length > 0) {
                    data = updateKeys({ data, keyMapping: columnKeyMappings, columns, userDateFormat, userDateTimeFormat: dateTimeFormat, userTimezoneOffset });
                    jsonResponse = data;
                }
            ...
            case mimeTypes.xlsx:
                ...
                if (data.length > 0) {
                    if (exportColumns && Object.keys(exportColumns)?.length > 0) {
                        data = updateKeys({ data, keyMapping: columnKeyMappings, columns, isForXML: true, userDateFormat, userDateTimeFormat: dateTimeFormat, userTimezoneOffset });
                    }
                    res.set('Content-Type', responseType);
                    res.set('Content-Disposition', `attachment; filename="${fileName}.xlsx"`);
                    return await toExcel({ title: "Main", columns, rows: data, stream: res, exportColumns: true, userDateFormat, userDateTimeFormat: dateTimeFormat, userTimezoneOffset });
                }
                break;
            case mimeTypes.txt:
                break;
