    ...
    res.transform = async function ({ success, ...others }, { responseType, fileName, data: dataField = "data" } = {}) {
        ...
        switch (responseType) {
            case mimeTypes.csv:
                ...
                if (data.length > 0) {
                    if (exportColumns && Object.keys(exportColumns)?.length > 0) {
                        data = updateKeys({ data, keyMapping: columnKeyMappings, columns, userDateFormat, userDateTimeFormat: userDateFormat + util.dateTimeCSVExportFormat });
                    }
            ...
            case mimeTypes.json:
                ...
                if (exportColumns && Object.keys(exportColumns)?.length > 0) {
                    data = updateKeys({ data, keyMapping: columnKeyMappings, columns, userDateFormat, userDateTimeFormat: dateTimeFormat });
                    jsonResponse = data;
                }
            ...
            case mimeTypes.xlsx:
                ...
                if (data.length > 0) {
                    if (exportColumns && Object.keys(exportColumns)?.length > 0) {
                        data = updateKeys({ data, keyMapping: columnKeyMappings, columns, isForXML: true, userDateFormat, userDateTimeFormat: dateTimeFormat });
                    }
                    res.set('Content-Type', responseType);
