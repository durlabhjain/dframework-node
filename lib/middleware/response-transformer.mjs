...
const updateLookups = ({ data, exportColumns, lookups }) => {
    ...
    const lookupKeys = new Set(Object.keys(lookups)); // Use a Set for faster lookup

    // Create a map of columns that have matching headers in lookupKeys
    const headerToFieldMap = columnsArray
        .filter(column => lookupKeys.has(column.headerName))
        .reduce((map, column) => {
            map[column.headerName] = column.field;
            return map;
        }, {});
    ...
    data.forEach(dataItem => {
        Object.entries(headerToFieldMap).forEach(([headerName, field]) => {
            const lookupItem = lookups[headerName]?.find(
