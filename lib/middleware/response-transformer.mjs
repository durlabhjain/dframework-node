...
const responseTransformer = async function (req, res, next) {
    res.transform = async function ({ success, ...others }, { responseType, fileName, data: dataField = "data" } = {}) {
        ...
        const exportColumns = others?.exportColumns, userDateFormat = others?.userDateFormat, isElastic = others?.isElastic, userTimezoneOffset = others?.userTimezoneOffset, lookups = others?.lookups, lookupFields = others?.lookupFields, addExecutionTimeLogger = others?.addExecutionTimeLogger, serverUrl = others?.serverUrl;
        ...
        if (isExportOperation) {
            ...
            const hasHyperlinkColumns = Object.values(exportColumns).some(col => col.hyperlinkURL && col.hyperlinkIndex);
            const rawRows = hasHyperlinkColumns && Array.isArray(data) ? data.map(row => ({ ...row })) : null;
            ...
            if (exportColumns && Object.keys(exportColumns)?.length > 0) {
                sheets = isMultiSheetExport ? data : [{ title: "Main", columns, rows: data, rawRows }];
        ...
        switch (responseType) {
            ...
            case mimeTypes.xlsx:
                ...
                return await toExcel({ sheets, stream: res, exportColumns: true, userDateFormat, userDateTimeFormat: dateTimeFormat, userTimezoneOffset, lookups, lookupFields, addExecutionTimeLogger, serverUrl });
            case mimeTypes.csv:
                if (data.length > 0) {
                    data = sanitizeData({ data, columns });
