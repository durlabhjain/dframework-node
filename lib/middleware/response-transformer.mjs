...
const updateKeys = function ({ data, keyMapping, columns, isForXML = false, userDateFormat, userDateTimeFormat, userTimezoneOffset, lookups, lookupFields = {} }) {
    ...
    logger.info(`Execution time taken for updateKeys: ${endTime - startTime} ms`);
...
const updateLookups = ({ data, exportColumns, lookups = {} }) => {
    ...
    logger.info(`Execution time taken for (columnsArray): ${performance.now() - block1Start} ms`);
    ...
    logger.info(`Execution time taken for (newLookups): ${performance.now() - block2Start} ms`);

    const block3Start = performance.now();
    ...
    logger.info(`Execution time taken for (headerToFieldMap): ${performance.now() - block3Start} ms`);

    const block4Start = performance.now();
    ...
    logger.info(`Execution time taken for (data value update): ${performance.now() - block4Start} ms`);

    const block5Start = performance.now();
    if (Array.isArray(data)) {
        data.forEach(item => {
    ...
    logger.info(`Execution time taken for (special fields): ${performance.now() - block5Start} ms`);

    const endTime = performance.now();
    logger.info(`Execution time taken for updateLookups: ${endTime - startTime} ms`);
};
