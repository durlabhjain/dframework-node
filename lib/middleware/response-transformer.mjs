 ...
 */
const sanitizeData = ({ data, columns = {} }) => {
    ...
    return data.map(record =>
        fieldsToKeep.reduce((acc, field) => {
            const name = columns[field]?.name || field;
    ...
    res.transform = async function ({ success, ...others }, { responseType, fileName, data: dataField = "data" } = {}) {
        ...
        data = data.map(record => {
            ...
            for (const [key, value] of Object.entries(record)) {
                ...
                if (value instanceof Date && exportColumns[key]) {
                    formattedRecord[key] = formatDateTime({ value, format: exportColumns[key].type === 'date' ? dateFormat : dateTimeFormat });
                    continue;
                }
