...
const updateKeys = function ({ data, keyMapping, columns, isForXML = false, userDateFormat, userDateTimeFormat, formatDate = false }) {
    ...
    const updatedObject = data.map(record => {
        ...
        for (const ele of Object.keys(keyMapping)) {
            ...
            const { valueType, keepLocal } = columns[ele];
            ...
            if (util.dateTimeFields.includes(valueType)) {
                if (value !== null && value !== undefined) {
                    ...
                    value = formatDate ? formatDateTime({ value, format: valueType === 'date' ? userDateFormat : userDateTimeFormat }) : value;
                } else {
                    value = '';
                }
...
const responseTransformer = async function (req, res, next) {
    res.transform = async function ({ success, ...others }, { responseType, fileName, data: dataField = "data" } = {}) {
        let data = success === true ? others[dataField] : null;
        const exportColumns = others?.exportColumns, userDateFormat = others?.userDateFormat, isElastic = others?.isElastic;
        const dateTimeFormat = userDateFormat + util.dateTimeExportFormat
        if (!success) {
            return res.status(400).json({ success: false, message: data });
        }

        fileName = `${!fileName ? req.path.substr(1).replace(/[^A-Za-z0-9-_]/g, '-') : fileName}-${dateFormat(new Date(), "isoDateTime").replace(/:/g, '')}`;
        ...
        for (const key in exportColumns) {
            ...
            const commonProperties = { width: exportColumn.width / util.excelColumnWidthEnum, name: exportColumn.headerName, valueType: exportColumn.type, keepLocal: exportColumn.keepLocal, isParsable: exportColumn.isParsable };
        ...
        switch (responseType) {
            case mimeTypes.csv:
                ...
                if (data.length > 0) {
                    if (exportColumns && Object.keys(exportColumns)?.length > 0) {
                        data = updateKeys({ data, keyMapping: columnKeyMappings, columns, userDateFormat, userDateTimeFormat: userDateFormat + util.dateTimeCSVExportFormat, formatDate: true });
                    }
            ...
            case mimeTypes.json:
                ...
                if (exportColumns && Object.keys(exportColumns)?.length > 0) {
                    data = updateKeys({ data, keyMapping: columnKeyMappings, columns, userDateFormat, userDateTimeFormat: dateTimeFormat, formatDate: true });
                    jsonResponse = data;
                }
            ...
            case mimeTypes.xlsx:
                ...
                if (data.length > 0) {
                    if (exportColumns && Object.keys(exportColumns)?.length > 0) {
                        data = updateKeys({ data, keyMapping: columnKeyMappings, columns, isForXML: true, userDateFormat, userDateTimeFormat: dateTimeFormat, formatDate: false });
                    }
                    res.set('Content-Type', responseType);
                    res.set('Content-Disposition', `attachment; filename="${fileName}.xlsx"`);
                    return await toExcel({ title: "Main", columns, rows: data, stream: res, exportColumns: true, userDateFormat, userDateTimeFormat: dateTimeFormat });
                }
                break;
            case mimeTypes.txt:
                break;
            case mimeTypes.xml:
                ...
                if (exportColumns && Object.keys(exportColumns)?.length > 0) {
                    data = updateKeys({ data, keyMapping: columnKeyMappings, columns, isForXML: true, userDateFormat, userDateTimeFormat: dateTimeFormat, formatDate: true });
                }
