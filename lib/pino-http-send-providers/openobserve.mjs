...
import { MACHINE_NAME, CWD, extractCommonFields } from "./shared.mjs";
...
const formatUtcDate = (time) => dayjs.utc(time).format("YYYY-MM-DD hh:mm:ss A");
...
const buildRecord = (log, context) => {
  ...
  const {
    msg, message, err, query,
    executionTimeMs, durationMs, durMs,
    formattedQuery,
    method, url,
    stack,
  } = rest;

  // dframework's own slow-query (sql.js) logs carry a real `stack` captured at the call
  // site; slow-request (slow-request-logger.mjs) logs have neither `err` nor `stack`, so
  // fall back to whatever diagnostic fields they do carry instead of leaving it blank.
  // Duration is only synthesized in that last-resort case -- when a real stack (or err)
  // exists, the duration is already part of the log message, so repeating it here would
  // just be noise. Parameters aren't repeated here since formattedQuery already has them
  // inlined, and body/form are already passed separately below.
  const sqlQuery = formattedQuery || query;
  let stackTrace = "";
  if (err) {
    stackTrace = `${err.stack || err.message || ""}${sqlQuery ? `\r\nquery: ${sqlQuery}` : ""}`;
  } else if (stack) {
    stackTrace = `${stack}${sqlQuery ? `\r\nquery: ${sqlQuery}` : ""}`;
  } else {
    const durationValue = executionTimeMs ?? durationMs ?? durMs;
    const lines = [];
    if (sqlQuery) lines.push(`query: ${sqlQuery}`);
    if (!sqlQuery && (req.url || url)) lines.push(`${method || req.method || ""} ${req.url || url}`.trim());
    if (durationValue !== undefined) lines.push(`duration: ${durationValue}ms`);
  ...
  return withoutEmpty({
    ...
    form: req.params,
    body: (req.body !== null && req.body !== undefined) ? JSON.stringify(req.body) : req.body,
