...
const buildRecord = (log, context) => {
  ...
  // dframework's own slow-query (sql.js) and slow-request (slow-request-logger.mjs) logs
  // have no `err`, so build a stack_trace from whatever diagnostic fields they do carry
  // instead of leaving it blank. Parameters aren't repeated here since formattedQuery
  // already has them inlined, and body/form are already passed separately below.
