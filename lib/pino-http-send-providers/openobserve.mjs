...
const buildRecord = (log, context) => {
  ...
  const { msg, message, stack, query, formattedQuery, ...details } = rest;
  const { err } = details;
  const stackTrace = err?.stack || stack || err?.message || "";
  // Prefer formattedQuery (literal parameter values inlined) over the raw query text.
  const queryText = formattedQuery || query || "";
  ...
  return withoutEmpty({
    ...
    // Promoted to its own searchable field instead of being buried in details.
    query: queryText,
    // Keep arbitrary caller data without interpreting domain-specific fields or
    // allowing them to overwrite the provider's metadata. JSON text also keeps
    // the ingest field type stable across unrelated log payloads.
