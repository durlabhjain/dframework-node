  ...
  const flushBatch = async (logs) => {
    ...
    if (logs.length > 1 && typeof provider.buildBatchRequest === "function") {
      try {
        ...
        console.log(`[pino-http-send] Sent batch of ${logs.length} log(s) to ${baseURL.href}`);
      } catch (err) {
        console.error(`[pino-http-send] Failed to send batch of ${logs.length} log(s) to ${baseURL.href}: ${err.message}`);
      }
      return;
    }
    ...
    for (const log of logs) {
      try {
        ...
        await sendRequest({ ...(options.headers || {}), ...headers }, body);
        console.log(`[pino-http-send] Sent log "${log.msg || ""}" to ${baseURL.href}`);
      } catch (err) {
        console.error(`[pino-http-send] Failed to send log "${log.msg || ""}" to ${baseURL.href}: ${err.message}`);
      }
    }
  };
