    ...
    return build(
        async (source) => {
            ...
            for await (let line of source) {
                ...
                // Save log string to file using fs-extra
                const logString = `Sending log to ${options.url} with params: ${formParams.toString()} for request ${rawUrl ?? (req.url || "")} with headers: ${JSON.stringify(headers)}`;
                const logFilePath = path.resolve('error-email-logging');
