...
const stringifyWithMultilineFields = (value) => {
  ...
  placeholders.forEach((original, i) => {
    // Preserve existing real whitespace; only de-escape \n/\t/\r when the string looks double-escaped.
    const withWhitespace =
      /\\[ntr]/.test(original) && !/[\n\t\r]/.test(original)
        ? original.replace(/\\n/g, "\n").replace(/\\t/g, "\t").replace(/\\r/g, "\r")
        : original;
    const escaped = withWhitespace.replace(/\\/g, "\\\\").replace(/"/g, '\\"');
    text = text.split(`"@@MULTILINE_${i}@@"`).join(`"${escaped}"`);
  });
  return text;
};
