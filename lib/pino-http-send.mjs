...
const createWriteStream = function (options = {}) {
  ...
  const flushBatch = async (logs) => {
    ...
    // Send each log individually in the primary no-batch path, and also as a
    // fallback after a batched send fails, so a single bad batch does not drop the
    // whole buffer while still surfacing the underlying transport error.
