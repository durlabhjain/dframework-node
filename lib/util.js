...
export default {
    ...
    /**
	* Remove all special characters from a string, keeping only:
	* - A–Z
	* - a–z
	* - internal spaces (not at start or end)
	* This function sanitizes a given input string by:
	* - Removing all characters except letters and spaces.
	* - Trimming spaces at the beginning and end of the string.
	* - Preserving only internal spaces between words.
	* Behavior:
	* - Accepts a single string as input.
	* - Removes special characters, digits, punctuation, etc.
	* - Keeps internal spaces.
	* - Trims leading and trailing spaces after cleaning.
	* Examples:
	* - sanitizeString(" Hello World! ") → "Hello World"
	* - sanitizeString("123 Test 456") → "Test"
	* - sanitizeString(" ###ABC### ") → "ABC"
	* @param {string} str - Input string to sanitize.
	* @returns {string} - Sanitized, trimmed string.
	*/
	sanitizeString: function (str) {
		// If input is not a string, handle gracefully
		if (typeof str !== "string") {
			return "";
		}

		// 1. Remove everything except letters and spaces
