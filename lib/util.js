...
export default {
    ...
    createProxy: (originalFunction, logQueryTime, queryLogThreshold, timeoutLogLevel, logger) => {
        return new Proxy(originalFunction, {
            apply: async function (target, thisArg, args) {
                const queryStartTime = Date.now();
                const returnValue = await Reflect.apply(target, thisArg, args);
                const [query] = args;
                logQueryTime({ query, start: queryStartTime, end: Date.now(), parameters: thisArg.parameters, queryLogThreshold, timeoutLogLevel, logger });
                return returnValue;
            }
        });
    }
