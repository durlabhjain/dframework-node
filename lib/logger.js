...
const hasCustomLevels = Object.keys(customLevels).length > 0;

// A custom level like "slow" is defined specifically so slow-query/slow-request logs are
// observable, so default the HTTP sink to include it too -- same as the dedicated file
// transport below already does automatically -- instead of silently dropping everything
// under "error" unless the app sets postLevel explicitly.
const defaultPostLevel = hasCustomLevels
    ? Object.keys(customLevels)
        .filter((name) => customLevels[name] < pino.levels.values.error)
        .sort((a, b) => customLevels[a] - customLevels[b])[0] || "error"
    : "error";

const loggingConfig = {
    postLevel: defaultPostLevel,
    stdout: true,
    logLevel: 'info',
    logFolder: './logs',
    mixin: null,
...
const mergedLevels = hasCustomLevels ? { ...pino.levels.values, ...customLevels } : pino.levels.values;
...
const logger = pino({
    ...
    targets: transports,
    levels: mergedLevels,
    // HTTP is an independent sink: a higher-level file target must not suppress it.
    dedupe: !otherConfig.httpConfig,
