...
class Sql {
    ...
    in({ request, fieldName, paramName, values, ignoreZero = false, sqlType = this.dataTypes.integer, operator = "=", useTvp = false, tvpType, tvpColumnName }) {
        ...
        const isNumeric = this.dataTypes.numeric.includes(sqlType);
        const hasDecimals = this.dataTypes.withDecimals.includes(sqlType);
        let tvp;
        if (useTvp) {
            if (!this.allowTvp) {
    ...
    addParameters({ query, request, parameters, forWhere = false }) {
        ...
        for (let index = 0, len = paramNames.length; index < len; index++) {
            ...
            if (Array.isArray(value) || ["in", "not in"].includes(operator.toLowerCase())) {
                const inResult = isBetweenOperator ? this.between({ request, fieldName, paramName, values: value, operator, sqlType: sqlType || this.dataTypes.integer }) : this.in({ request, fieldName, paramName, values: value, operator, sqlType: sqlType || this.dataTypes.integer });
    ...
    async join({ query, matchColumn, matchValue, rows }) {
        ...
        for (const id in ids) {
            ...
            request.input(paramName, this.dataTypes.integer, ids[id].key);
