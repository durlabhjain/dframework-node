    ...
    addParameters({ query, request, parameters, forWhere = false }) {
        ...
        for (let index = 0, len = paramNames.length; index < len; index++) {
            ...
            if (Array.isArray(value) || ["in", "not in"].includes(operator.toLowerCase())) {
                const sqlDataType = sqlType ? sqlDataTypes[sqlType] : mssql.Int;
                const inResult = isBetweenOperator ? this.between({ request, fieldName, paramName, values: value, operator, sqlType: sqlDataType }) : this.in({ request, fieldName, paramName, values: value, operator, sqlType: sqlDataType });
