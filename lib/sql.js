...
class Sql {
    ...
    async logQueryTime({ query, start, end = Date.now(), parameters, queryLogThreshold, timeoutLogLevel, logger }) {
    ...
    async execute({ query, parameters, where, orderBy, isStoredProcedure }) {
        ...
        try {
            ...
            this.logQueryTime({ query, start: queryStartTime, end: Date.now(), value: parameters || where });
    ...
    createRequest() {
        ...
        const { logQueryTime, queryLogThreshold, timeoutLogLevel, logger } = this;

        request.oldQuery = request.query;
        request.query = util.createProxy(request.oldQuery, logQueryTime, queryLogThreshold, timeoutLogLevel, logger);

        request.oldExecute = request.execute;
        request.execute = util.createProxy(request.oldExecute, logQueryTime, queryLogThreshold, timeoutLogLevel, logger);
        return request;
    }
