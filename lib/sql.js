...
class Sql {
    ...
    async setConfig({ logger, timeoutLogLevel = "info", queryLogThreshold = 1000, forceCaseInsensitive, inOperatorStrategy = "innerJoin", ...config } = {}) {
        if (logger) {
            this.logger = logger;
        }
     ...
     * @param {String} config.strategy - override the default inOperatorStrategy ("innerJoin", "exists", "in")
     * @returns
     */
    in({ request, fieldName, paramName, values, ignoreZero = false, sqlType = this.dataTypes.integer, operator = "=", useTvp = false, tvpType, tvpColumnName, useSequence = true, strategy }) {
        ...
        // Validate strategy
        const validStrategies = ['innerJoin', 'exists', 'in'];
        ...
        if (useTvp) {
            ...
            const tvpAlias = this.getNextTvpAlias();
            
            // Generate statement based on strategy
            if (operatorStrategy === 'innerJoin' && operator === 'IN') {
                statement = `INNER JOIN ${buildParameterName(paramName)} AS ${tvpAlias} ON ${fieldName} = ${tvpAlias}.${tvpColumnName}`;
            } else if (operatorStrategy === 'exists' && operator === 'IN') {
                statement = `EXISTS (SELECT 1 FROM ${buildParameterName(paramName)} AS ${tvpAlias} WHERE ${fieldName} = ${tvpAlias}.${tvpColumnName})`;
            } else if (operatorStrategy === 'exists' && operator === 'NOT IN') {
                statement = `NOT EXISTS (SELECT 1 FROM ${buildParameterName(paramName)} AS ${tvpAlias} WHERE ${fieldName} = ${tvpAlias}.${tvpColumnName})`;
            } else if (operatorStrategy === 'innerJoin' && operator === 'NOT IN') {
                // For NOT IN with INNER JOIN, we need to use NOT EXISTS or LEFT JOIN with NULL check
                // NOT EXISTS is more straightforward
                statement = `NOT EXISTS (SELECT 1 FROM ${buildParameterName(paramName)} AS ${tvpAlias} WHERE ${fieldName} = ${tvpAlias}.${tvpColumnName})`;
            } else {
                // Default to IN/NOT IN operator
                statement = `${fieldName} ${operator} (SELECT ${tvpColumnName} FROM ${buildParameterName(paramName)})`;
            }
            
            return { paramNames: [paramName], values: [tvp], statement, alias: tvpAlias };
        }

        // For non-TVP scenarios (individual parameters)
        if (operatorStrategy === 'innerJoin' && operator === 'IN') {
            // Create a derived table with the values
            const tvpAlias = this.getNextTvpAlias();
            const valuesList = paramNames.map(p => `SELECT ${p} AS Value`).join(' UNION ALL ');
            statement = `INNER JOIN (${valuesList}) AS ${tvpAlias} ON ${fieldName} = ${tvpAlias}.Value`;
        } else if (operatorStrategy === 'exists' && operator === 'IN') {
            const tvpAlias = this.getNextTvpAlias();
            const valuesList = paramNames.map(p => `SELECT ${p} AS Value`).join(' UNION ALL ');
            statement = `EXISTS (SELECT 1 FROM (${valuesList}) AS ${tvpAlias} WHERE ${fieldName} = ${tvpAlias}.Value)`;
        } else if (operatorStrategy === 'exists' && operator === 'NOT IN') {
            const tvpAlias = this.getNextTvpAlias();
            const valuesList = paramNames.map(p => `SELECT ${p} AS Value`).join(' UNION ALL ');
            statement = `NOT EXISTS (SELECT 1 FROM (${valuesList}) AS ${tvpAlias} WHERE ${fieldName} = ${tvpAlias}.Value)`;
        } else if (operatorStrategy === 'innerJoin' && operator === 'NOT IN') {
            // For NOT IN with INNER JOIN, use NOT EXISTS
            const tvpAlias = this.getNextTvpAlias();
            const valuesList = paramNames.map(p => `SELECT ${p} AS Value`).join(' UNION ALL ');
            statement = `NOT EXISTS (SELECT 1 FROM (${valuesList}) AS ${tvpAlias} WHERE ${fieldName} = ${tvpAlias}.Value)`;
        } else {
