...
class Sql {
     ...
     * - `'ilike'`: uses the `ILIKE` / `NOT ILIKE` operator (e.g. Starrocks, PostgreSQL).
    ...
    async setConfig({ logger, timeoutLogLevel = "info", queryLogThreshold = 1000, forceCaseInsensitive, inOperatorStrategy = inOperatorStrategies.INNER_JOIN, caseInsensitiveMode, shadowColumns, ...config } = {}) {
        if (logger) {
            this.logger = logger;
        }
