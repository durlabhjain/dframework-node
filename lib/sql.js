    ...
    async runQuery({ request, type = "query", query }) {
        try {
            ...
            const executionTime = endTime - startTime;
             if (executionTime > 6000) { // 6 seconds in milliseconds
                logger.warn({
                    message: 'Query execution exceeded 6 seconds' ,
                    query,
            ...
            logger.error({ err: { stack: query, message: err.message }, parameters: request.parameters, type });
