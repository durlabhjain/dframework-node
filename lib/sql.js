...
class Sql {
    ...
    async setConfig({ logger, timeoutLogLevel = "info", queryLogThreshold = 1000, forceCaseInsensitive, inOperatorStrategy = inOperatorStrategies.INNER_JOIN, maxRetries = 3, retryBaseDelay = 5000, ...config } = {}) {
        if (logger) {
            this.logger = logger;
        }
        ...
        const errorHandler = (err) => {
            this.logger.error({ message: 'SQL pool error', err });
            this._reconnect().then(() => {
                if (this.pool) {
                    this.pool.on('error', errorHandler);
                }
            }).catch((reconnectErr) => {
                this.logger.error({ message: 'SQL pool reconnection failed', err: reconnectErr });
            });
        };
    ...
    async createPoolConnection(config) {
        ...
        const { maxRetries, retryBaseDelay } = this;
        let lastError;
        for (let attempt = 0; attempt <= maxRetries; attempt++) {
            try {
                ...
                if (attempt < maxRetries) {
                    this.logger.warn({
                        message: `SQL connection attempt ${attempt + 1} failed, retrying in ${retryBaseDelay}ms`,
                        err
                    });
                    await new Promise(resolve => setTimeout(resolve, retryBaseDelay));
                }
            }
        }
        this.logger.error({ message: `SQL connection failed after ${maxRetries + 1} attempts`, err: lastError });
        throw lastError;
    }

    async _reconnect() {
        this.logger.warn({ message: 'SQL pool disconnected, attempting to reconnect...' });
        const oldPool = this.pool;
        try {
            this.pool = await this.createPoolConnection(this._dbConfig);
            this.logger.info({ message: 'SQL pool reconnected successfully' });
        } catch (err) {
            this.logger.error({ message: 'SQL pool reconnection failed', err });
            throw err;
        } finally {
            if (oldPool) {
                try {
                    if (typeof oldPool.removeAllListeners === 'function') {
                        oldPool.removeAllListeners();
                    }
                    if (typeof oldPool.close === 'function') {
                        await oldPool.close();
                    }
                } catch (closeErr) {
                    this.logger.warn({ message: 'Error closing old SQL pool during reconnection', err: closeErr });
                }
            }
        }
    }
    ...
    addParameters({ query, request, parameters, forWhere = false }) {
        ...
        for (let index = 0, len = paramNames.length; index < len; index++) {
            ...
            let sqlType, useTvp, tvpType, tvpColumnName, useSequence, strategy;
