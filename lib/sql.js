...
class Sql {
            ...
            if(util.useSqlParameterLength) {
                ...
                for(const parameter of Object.values(request.parameters)) {
                    switch(parameter.type) {
                        case mssql.VarChar: 
                            parameter.length = isNaN(parameter.length) ? sqlParameterLength.varchar : parameter.length;
                            break;
                        case mssql.NVarChar:
                            parameter.length = isNaN(parameter.length) ? sqlParameterLength.nvarchar : parameter.length;
                            break;
                        case mssql.Decimal:
                            parameter.scale = isNaN(parameter.scale) ? sqlParameterLength.decimal_scale : parameter.scale;
                            parameter.precision = isNaN(parameter.precision) ? sqlParameterLength.decimal_precision : parameter.precision;
                            break;
                    }
                }
            }
    ...
    async insertUpdate({ tableName, json, keyField, update = false }) {
        const { buildParameterName, insertedIdStatement, createRequest, runQuery } = this;
    ...
    async join({ query, matchColumn, matchValue, rows }) {
        const { buildParameterName, dataTypes, createRequest, runQuery } = this;
        const ids = {};
