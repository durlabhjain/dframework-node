...
class Sql {
    ...
    applyCaseInsensitive({ fieldName, value, operator, type, sqlType }) {
        ...
        if (caseInsensitiveMode === 'ilike-fn') {
            ...
            const posOps = new Set(['=', 'LIKE', 'CONTAINS', 'STARTSWITH', 'ENDSWITH']);
            const negOps = new Set(['!=', '<>', 'NOT LIKE', 'NOT CONTAINS']);
            ...
            if (forceCaseInsensitive) {
                ...
                if (!isDateType) {
                    ...
                    if (result.statementTemplate) {
                        ...
                        const stmt = result.statementTemplate.replace('{param}', buildParameterName(paramName));
                        if (sqlType !== undefined && sqlType !== null) {
