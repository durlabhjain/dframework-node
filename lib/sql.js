...
class Sql {
    ...
    async setConfig({ logger, timeoutLogLevel = "info", queryLogThreshold = 1000, forceCaseInsensitive, inOperatorStrategy = inOperatorStrategies.INNER_JOIN, caseInsensitiveMode, shadowColumns, caseInsensitiveOrderBy, ...config } = {}) {
        if (logger) {
            this.logger = logger;
        }
    ...
    addParameters({ query, request, parameters, forWhere = false, logicalOperator = 'AND', appendAnd = false }) {
        ...
        for (let index = 0, len = paramNames.length; index < len; index++) {
            ...
            if (Array.isArray(value) || ["in", "not in"].includes(operator.toLowerCase())) {
                const inResult = isBetweenOperator ? this.between({ request, fieldName, paramName, values: value, operator, sqlType: sqlType || dataTypes.integer }) : this.in({ request, fieldName, paramName, values: value, operator, sqlType: sqlType || dataTypes.integer, useTvp, tvpType, tvpColumnName, useSequence, strategy });
