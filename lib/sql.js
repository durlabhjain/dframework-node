...
class Sql {
    ...
    async logQueryTime({ query, start, end = Date.now(), value }) {
        const queryDurationInMs = (end - start);
        if (queryDurationInMs > this.queryLogThreshold) {
            this.logger[this.timeoutLogLevel]({
                query: query,
    ...
    createRequest() {
        ...
        request.query = async (query) => {
            const queryStartTime = Date.now();
            const returnValue = await request.oldQuery(query);
            this.logQueryTime({ query, start: queryStartTime, end: Date.now(), value: Object.keys(request.parameters).length !== 0 ? request.parameters : '' });
            return returnValue;
        }
        return request;
    }
