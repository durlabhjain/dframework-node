...
class Sql {
    ...
    async runQuery({ request, type = "query", query }) {
        try {
            ...
            logger.error({ err: { stack: err.stack, message: err.message, error: err, query }, parameters: request.parameters, type });
        return { success: false, err, data: {} };
        }
    }

    static logSlowQuery({ startTime, query, type, request }) {
     ...
     */
    async execute({ query, parameters, where, orderBy, isStoredProcedure, request }) {
        ...
        const req = request || this.createRequest();
        query = this.addParameters({ query, request: req, parameters, forWhere: false });
        query = this.addParameters({ query, request: req, parameters: where, forWhere: true });

        if (orderBy) {
            query += " ORDER BY " + orderBy;
        }
        const type = isStoredProcedure ? "execute" : "query";
        return this.runQuery({ request: req, type, query });
    }
