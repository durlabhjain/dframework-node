...
class Sql {
    ...
    in({ request, fieldName, paramName, values, ignoreZero = false, sqlType = mssql.Int, operator = "=", useTvp = false, tvpType, tvpColumnName }) {
        ...
        if (useTvp) {
            ...
            const tvp = new mssql.Table(tvpType);
        }
     ...
     */
    addParameters({ query, request, parameters, forWhere = false , logicOperator = 'AND'}) {
        if (!parameters) {
            return query;
        }
        ...
        if (whereClauses.length > 0) {
            ...
            if (whereClauses.length == 1) {
                ...
                // Use the reduce() method to join the array with conditional delimiters
                const joinedClauses = whereClauses.reduce((result, clause, index) => {
                    // Use the first delimiter for the first element, and the subsequent delimiter for the rest
                    const delimiter = index === 0 ? ' AND ' : ` ${index == whereClauses.length-1 ? " " : logicOperator.toUpperCase()} `;
                    // Append the clause to the result with the appropriate delimiter
                    return result + clause + delimiter;
                }, "");
                query += " WHERE " + joinedClauses;
            }
        }
        return query;
    }
