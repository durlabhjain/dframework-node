            ...
            if(util.useSqlParameterLength) {
                ...
                for(const parameter of Object.values(request.parameters)) {
                    switch(parameter.type) {
                        case mssql.VarChar: 
                            parameter.length = !isNaN(parameter.length) ? parameter.length : sqlParameterLength.varchar;
                            break;
                        case mssql.NVarChar:
                            parameter.length = !isNaN(parameter.length) ? parameter.length : sqlParameterLength.nvarchar;
                            break;
                        case mssql.Decimal:
                            parameter.scale = !isNaN(parameter.scale) ? parameter.scale : sqlParameterLength.decimal_scale;
                            parameter.precision = !isNaN(parameter.precision) ? parameter.precision : sqlParameterLength.decimal_precision;
                            break;
                        case mssql.Date:
                            parameter.type = sqlParameterLength.useDateTime2 ? mssql.DateTime2.type : parameter.type;
                            break;
                    }
                }
            }
