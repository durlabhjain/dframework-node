...
const { reportDatesFormat, reportTypes, dateTimeExcelExportFormat, dateTimeExportFormat } = util;
...
const resolveFilePath = ({ filePath, title, outputPath, extension }) => {
    ...
    const safeTitle = util.sanitizeFilename(title || 'report');
 ...
 * @param {string} configuration.fileName - @deprecated since 3.1.0. Use filePath instead.
 * @param {string} configuration.outputPath - Output folder (default: from config.defaultReportPath or '.'). Used only when filePath is not provided.
 */
const toExcel = async function ({ title, rows, columns, stream, sheets, filePath, fileName, exportColumns = false, userDateFormat, userTimezone, outputPath }) {
    // Use config.defaultReportPath if outputPath not provided
    if (outputPath === undefined) {
        outputPath = config?.defaultReportPath || '.';
    }

    // Warn if fileName contains an extension (common migration issue)
    if (fileName && (fileName.toLowerCase().endsWith('.xlsx') || fileName.toLowerCase().endsWith('.xls'))) {
        logger.warn({ fileName }, 'fileName parameter should not include the .xlsx extension. The extension is automatically added. Use filePath parameter for full path control, or pass fileName without extension.');
    }
    ...
    for (const sheetDetail of sheets) {
        await writeExcelSheet({ title, ...sheetDetail, workbook, exportColumns, userDateFormat, userTimezone });
    }
    if (stream) {
...
const writeExcelSheet = function ({ title = "main", rows, columns, name, workbook, tableName, exportColumns = false, userDateFormat, userTimezone }) {
        ...
        if (colConfig) {
            ...
            if (Object.keys(others).length > 0) {
                if (['date', 'dateTime'].includes(others.valueType)) {
                    const effectiveDateFormat = userDateFormat || reportDatesFormat['date'];
                    const format = others.valueType === 'date' ? effectiveDateFormat : effectiveDateFormat + dateTimeExcelExportFormat;
    ...
    for (const row of rows) {
        ...
        for (const key of keys) {
            ...
            // Parse and convert date/dateTime values to Date objects
            if (value !== null && !(value instanceof Date) && colConfig && ['date', 'dateTime'].includes(colConfig.valueType)) {
                // For dateTime columns, always parse as UTC. For date columns, parse in local time.
            ...
            // Handle localization for date/dateTime columns
            if (value !== null && colConfig && ['date', 'dateTime'].includes(colConfig.valueType)) {
                if (colConfig.localize && userTimezone) {
                    // For localized dates, we need to convert from UTC to the user's timezone
                    // Since Excel doesn't support timezones, we shift the UTC time by the offset

                    /**
                     * Step 1: Parse the value as UTC (it's already a Date object at this point)
                     * - For 'dateTime' columns: the Date object represents UTC time
                     * - For 'date' columns: the Date object represents local time, so we treat it as UTC for consistency
                     */
                    const utcMoment = dayjs.utc(value);

                    /**
                     * Step 2: Get how this UTC time appears in the user's timezone
                     * - `.tz()` does NOT change the moment, only its representation
                     *   Example: 09:15 UTC → 02:45 PM IST (same instant)
                     */
                    const zoned = utcMoment.tz(userTimezone);

                    /**
                     * Step 3: Get timezone offset (in minutes) for this specific date
                     * - Needed for DST-aware zones where offset changes by date
                     *   Example: EST = -300 (winter), EDT = -240 (summer)
                     */
                    const offsetMinutes = zoned.utcOffset();

                    /**
                     * Step 4: Shift the time for Excel display
                     * - Excel has NO timezone support, only raw date-time values
                     * - So we manually convert UTC → local "wall-clock" time
                     *   Example: 09:15 UTC + 330 minutes = 14:45 (02:45 PM in IST)
                     */
                    value = utcMoment.add(offsetMinutes, 'minute').toDate();
                }
            }
    ...
    for (const [colNumber, key] of keys.entries()) {
        ...
        const { hyperlinkURL, hyperlinkIndex, valueType, localize } = columns[key] || {};
        // if (callback) {
        ...
        if (hyperlinkURL && hyperlinkIndex) {
            for (const [rowNumber] of rows.entries()) {
                ...
                const effectiveDateFormat = userDateFormat || reportDatesFormat['date'];
                const format = valueType === 'date' ? effectiveDateFormat : effectiveDateFormat + dateTimeExportFormat;
                let displayText;
                if (cellValue instanceof Date) {
                    let d = dayjs.utc(cellValue);
                    if (localize) {
                        d = dayjs(cellValue);
                        userTimezone ? d.tz(userTimezone) : d.local();
                    }
                    displayText = d.format(format);
                } else {
                    displayText = String(cellValue);
                }
                cell.value = { text: displayText, hyperlink: url };
...
const render = async function ({ reportName, title, rows, toFile, columns, sheets, reportType = defaultFileType, settings, exportColumns, outputPath, userDateFormat, userTimezone, logger: customLogger }) {
    // Use config.defaultReportPath if outputPath not provided
    if (outputPath === undefined) {
        outputPath = config?.defaultReportPath || '.';
    }
    ...
    if (toFile) {
        ...
        return reportHandler({ reportName, title, rows, columns, sheets, settings, reportType, exportColumns, outputPath, userDateFormat, userTimezone });
    }
...
const reports = {
    execute: async function ({ ReportType, options = {} }) {
        ...
        const outputPath = options.outputPath || options.reportFolder || config?.defaultReportPath || '.';
        const { userDateFormat, userTimezone } = options;
        const filePath = await render({ title: report.title, rows, toFile: true, columns, sheets, reportType, settings: report.reportSettings || {}, exportColumns: report.exportColumns || false, outputPath, userDateFormat, userTimezone, logger: customLogger });
        if (!filePath) {
            const message = `Failed to render report output for type: ${reportType}`;
            customLogger.error({ reportType, title: report.title, outputPath }, message);
            return { title: report.title, filePath: null, file: null, tags: report.tags || null, success: false, message };
        }
