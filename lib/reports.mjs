    ...
    for (const key of keys) {
        ...
        if (colConfig) {
            ...
            if (Object.keys(others).length > 0) {
                if (['date', 'dateTime'].includes(others.valueType) && !others.localize) {
        ...
        for (const key of keys) {
            ...
            if (colConfig && ['date', 'dateTime'].includes(colConfig.valueType)) {
                if (colConfig.localize) {
                    // TODO: Need to set date object instead of string
                    const effectiveDateFormat = userDateFormat || reportDatesFormat['date'];
                    const format = colConfig.valueType === 'date' ? effectiveDateFormat : effectiveDateFormat + dateTimeExportFormat;
