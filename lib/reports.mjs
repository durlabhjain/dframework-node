 ...
 * @param {string} configuration.fileName - Optional base file name without extension (not required if stream is used)
 * @param {string} configuration.reportFolder - Output folder (defaults to current directory)
 */
const toExcel = async function ({ title, rows, columns, stream, sheets, fileName, exportColumns = false, userDateFormat, reportFolder = '.', returnFullPath = false }) {
    ...
    if (stream) {
        ...
        // Callers must not include the extension; .xlsx is always appended
        const baseName = fileName || title;
        const fullPath = path.join(reportFolder, `${baseName}.xlsx`);
...
const toTextFile = async ({ title = "main", rows, stream, settings, reportType = reportTypes.csv, reportFolder = '.' }) => {
    let textFile;
    ...
    if (isJson) {
        await fse.writeJSON(path.join(reportFolder, `${title}.${reportType}`), rows);
        return;
    }
    await fse.writeFile(path.join(reportFolder, `${title}.${reportType}`), textFile)
}
...
const render = async function ({ reportName, title, rows, toFile, columns, sheets, reportType = defaultFileType, settings, exportColumns, reportFolder = '.', logger: customLogger }) {
    ...
    if (toFile) {
        ...
        await reportHandler({ reportName, title, rows, columns, sheets, settings, reportType, exportColumns, reportFolder });
...
const reports = {
    execute: async function ({ ReportType, options }) {
        ...
        await render({ title: report.title, rows, toFile: true, columns, sheets, reportType, settings: report.reportSettings || {}, exportColumns: report.exportColumns || false, reportFolder, logger: customLogger });
        // Callers must not include the extension in report.title; it is always appended here
        const fileName = `${report.title}.${reportType}`;
        return { title: report.title, filePath: path.join(reportFolder, fileName), file: fileName, tags: report.tags || null, success: true };
    }
};
