...
let reportsConfig = {
    defaultReportPath: config?.defaultReportPath || '.'
};
 ...
 */
const toExcel = async function ({ title, rows, columns, stream, sheets, filePath, fileName, exportColumns = false, userDateFormat, outputPath = reportsConfig.defaultReportPath }) {
...
const toTextFile = async ({ title = "main", rows, stream, settings, reportType = reportTypes.csv, filePath, outputPath = reportsConfig.defaultReportPath }) => {
    let textFile;
...
const render = async function ({ reportName, title, rows, toFile, columns, sheets, reportType = defaultFileType, settings, exportColumns, outputPath = reportsConfig.defaultReportPath, logger: customLogger }) {
...
/**
 * Set report configuration programmatically
 * @param {Object} config - Configuration object
 * @param {string} config.defaultReportPath - Default output folder for reports
 */
const setReportConfig = function (config) {
    Object.assign(reportsConfig, config);
}
...
const reports = {
    execute: async function ({ ReportType, options = {} }) {
        ...
        const outputPath = options.outputPath || options.reportFolder || reportsConfig.defaultReportPath;
