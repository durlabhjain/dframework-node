...
const resolveFilePath = ({filePath, title, outputPath, extension}) => {
    ...
    return path.join(outputPath, `${title}.${extension}`);
}
 ...
 */
const toExcel = async function ({ title, rows, columns, stream, sheets, filePath, exportColumns = false, userDateFormat, outputPath = '.' }) {
