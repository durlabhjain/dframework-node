...
const writeExcelSheet = function ({ title = "main", rows, columns, name, workbook, tableName, exportColumns = false, userDateFormat }) {
    ...
    for (const [colNumber, key] of keys.entries()) {
        ...
        //     }
        // }
        if (hyperlinkURL && hyperlinkIndex !== undefined) {
            const hyperlinkColIndex = typeof hyperlinkIndex === 'number'
                ? hyperlinkIndex
                : keys.findIndex((key) => key === hyperlinkIndex);
            if (hyperlinkColIndex !== -1) {
                for (const [rowNumber, row] of rows.entries()) {
                    if (!tableRows[rowNumber][hyperlinkColIndex]) continue;
                    const cell = worksheet.getCell(2 + rowNumber, 1 + hyperlinkColIndex);
                    const displayText = String(tableRows[rowNumber][hyperlinkColIndex]);
                    cell.value = { formula: `HYPERLINK("${hyperlinkURL.replace(/"/g, '""')}","${displayText.replace(/"/g, '""')}")` };
