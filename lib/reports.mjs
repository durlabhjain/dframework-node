...
const writeExcelSheet = function ({ title = "main", rows, columns, name, workbook, tableName, exportColumns = false, userDateFormat, userTimezone }) {
    ...
    for (const row of rows) {
        ...
        for (const key of keys) {
            ...
            if (['date', 'dateTime'].includes(colConfig.valueType)) {
                if (colConfig.localize) {
                    const format = colConfig.valueType === 'date' ? userDateFormat : userDateFormat + dateTimeExportFormat;
                    // TODO: Check if this is the correct way to handle this as we should pass date object instead of string
                    value = dayjs.utc(value).tz(userTimezone).format(format);
                }
            }
            rowData.push(value);
        }
    ...
    for (const [colNumber, key] of keys.entries()) {
        ...
        if (hyperlinkURL && hyperlinkIndex) {
            for (const [rowNumber] of rows.entries()) {
                ...
                const format = valueType === 'date' ? userDateFormat : userDateFormat + dateTimeExportFormat;
                const displayText = cellValue instanceof Date ? localize ? dayjs(cellValue).format(format) : dayjs.utc(cellValue).format(format) : String(cellValue);
