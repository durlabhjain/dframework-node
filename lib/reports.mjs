 ...
 */
const toExcel = async function ({ title, rows, columns, stream, sheets, fileName, exportColumns = false, userDateFormat, addExecutionTimeLogger = false, serverUrl = '' }) {
...
const writeExcelSheet = function ({ title = "main", rows, columns, name, workbook, tableName, exportColumns = false, userDateFormat, rawRows = null, serverUrl = '' }) {
    ...
    for (const [colNumber, key] of keys.entries()) {
        ...
        if (hyperlinkURL && hyperlinkIndex) {
            for (const [rowNumber] of rows.entries()) {
                ...
                let url = hyperlinkURL;
                if (hyperlinkIndex !== undefined) {
                    const sourceRow = rawRows ? rawRows[rowNumber] : rows[rowNumber];
                    const indexValue = sourceRow?.[hyperlinkIndex];
                    if (indexValue !== null && indexValue !== undefined) {
                        url = serverUrl + hyperlinkURL.replace('{0}', String(indexValue));
                    }
                }
