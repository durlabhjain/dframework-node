...
const writeExcelSheet = function ({ title = "main", rows, columns, name, workbook, tableName, exportColumns = false, userDateFormat, userTimezone }) {
    ...
    for (const row of rows) {
        ...
        for (const key of keys) {
            ...
            if (colConfig && ['date', 'dateTime'].includes(colConfig.valueType)) {
                if (colConfig.localize) {
                    let offsetMinutes = 0;

                    if (userTimezone) {
                        /**
                         * Step 1: Get how the UTC value appears in user's timezone
                         * - `.tz()` does NOT change the moment, only its representation
                         *   Example: 09:15 UTC → 02:45 PM IST (same instant)
                         */
                        const zoned = dayjs.utc(value).tz(userTimezone);

                        /**
                         * Step 2: Get timezone offset (in minutes) for this date
                         * - Needed for DST-aware zones (offset changes by date)
                         *   Example: EST = -300, EDT = -240
                         */
                        offsetMinutes = zoned.utcOffset();

                        /**
                         * Step 3: Shift time for Excel display
                         * - Excel has NO timezone support, only raw date-time
                         * - So we must convert UTC → local "wall-clock" time manually
                         *   Example: 09:15 UTC + 330 → 02:45 PM (IST in Excel)
                         */
                        value = dayjs.utc(value).add(offsetMinutes, 'minute').toDate();
                    }
                }
            }
            rowData.push(value);
