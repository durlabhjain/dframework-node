 ...
 */
const toExcel = async function ({ title, rows, columns, stream, sheets, fileName, exportColumns = false, userDateFormat, addExecutionTimeLogger = false }) {
    const workbook = new ExcelJS.Workbook();
    if (!sheets) {
        sheets = [{ title, rows, columns }];
    }
    const excelGenerationStartTime = performance.now();
    for (const [i, sheetDetail] of sheets.entries()) {
...
const writeExcelSheet = function ({ title = "main", rows, columns, name, workbook, tableName, exportColumns = false, userDateFormat }) {
    ...
    const blockWorkbookStart = performance.now();
    workbook = workbook || new ExcelJS.Workbook();
    const blockWorkbookEnd = performance.now();
    logger.info(`Execution time taken for workbook init: ${blockWorkbookEnd - blockWorkbookStart} ms`);
    ...
    const blockRowsStart = performance.now();
    const tableRows = [];
    ...
    const blockRowsEnd = performance.now();
    logger.info(`Execution time taken for rows setup: ${blockRowsEnd - blockRowsStart} ms`);

    const blockAddTableStart = performance.now();
    worksheet.addTable({
        name: tableName,
        ref: "A1",
        headerRow: true,
    ...
    const blockAddTableEnd = performance.now();
    logger.info(`Execution time taken for addTable: ${blockAddTableEnd - blockAddTableStart} ms`);

    const blockFormulaStart = performance.now();
    ...
    const blockFormulaEnd = performance.now();
    logger.info(`Execution time taken for formula setup: ${blockFormulaEnd - blockFormulaStart} ms`);

    const blockColFormatStart = performance.now();
    for (const colFormat of colFormats) {
        const { index: colIndex, ...others } = colFormat;
        const column = worksheet.getColumn(colIndex);
        Object.assign(column, others);
    }
    const blockColFormatEnd = performance.now();
    logger.info(`Execution time taken for column formatting: ${blockColFormatEnd - blockColFormatStart} ms`);

    const endTime = performance.now();
    logger.info(`Execution time taken for writeExcelSheet: ${endTime - startTime} ms`);
};
