...
class BusinessBase {
    ...
    async save({ id, relations, relationsObject, ...values }) {
        ...
        if (result.success) {
            ...
            if (Object.keys(multiSelectValues).length) {
                // some multi select columns have values
                for (const [colName, colValue] of Object.entries(multiSelectValues)) {
                    const multiSelectColumnConfig = multiSelectColumns[colName];
                    const newEntries = colValue.split(",").map(v => parseInt(v)).filter(v => v !== 0 && v > 0 && !isNaN(v));
                    const dataRes = await sql.query(`SELECT ${multiSelectColumnConfig.foreignKey} FROM ${multiSelectColumnConfig.tableName} where IsDeleted = 0 and ${this.keyField}=${id};`);
                    const existingEntries = dataRes.map(entry => parseInt(entry[multiSelectColumnConfig.foreignKey]));

                    // Find removed entries (items in existingEntries but not in newEntries)
                    const removedEntries = existingEntries.filter(entry => !newEntries.includes(entry));
                    // Find newly added entries (items in newEntries but not in existingEntries)
                    const newlyAddedEntries = newEntries.filter(entry => !existingEntries.includes(entry));

                    if (removedEntries.length) {
                        // Delete removed entries
                        await sql.query(`UPDATE ${multiSelectColumnConfig.tableName} SET IsDeleted = 1 WHERE ${this.keyField} = ${id} AND ${multiSelectColumnConfig.foreignKey} IN (${removedEntries.join(",")})`);
                    }
                    if (newlyAddedEntries.length) {
                        // Insert newly added entries
                        const insertParams = newlyAddedEntries.map(entry => `(${entry}, ${id})`).join(",");
                        await sql.query(`INSERT INTO ${multiSelectColumnConfig.tableName} (${multiSelectColumnConfig.foreignKey}, ${this.keyField}) VALUES ${insertParams}`);
                    }
                }
            }
    ...
    async lookupList({ scopeId }) {
        ...
        console.log(where, this.standardTable, listStatement, isStandard);
