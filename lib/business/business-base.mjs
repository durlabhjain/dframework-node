...
class BusinessBase {
    ...
    createWhere({ alias = "Main", filterDeleted = true } = {}) {
        ...
        if (filterDeleted && this.softDelete) {
    ...
    async load({ id, relations }) {
        ...
        for (const [columnName, columnConfig] of Object.entries(multiSelectColumns)) {
            let query = `SELECT ${columnConfig.foreignKey} FROM ${columnConfig.tableName} WHERE ${keyField}=${id}`;
            if (this.softDelete) {
                query += ' and IsDeleted = 0 ';
            }
    ...
    async save({ id, relations, relationsObject, ...values }) {
        ...
        let result = await sql.insertUpdate({ tableName, keyField, id, json: requestValues, update: isUpdate });
            ...
            try {
                if (Object.keys(multiSelectValues).length) {
                    for (const [colName, colValue] of Object.entries(multiSelectValues)) {
                        ...
                        let query = `SELECT ${projection} FROM ${tableName} where ${this.keyField}=${id}`;
                        if (this.softDelete) {
                            query += ' and IsDeleted = 0';
                        }
                        const dataRes = await sql.query(query);
                        ...
                        if (removedEntries.length && isUpdate) {
                            ...
                            if (this.softDelete) {
                                ...
                                await sql.query(`Delete from ${tableName} WHERE ${this.keyField} = ${id} AND ${foreignKey} IN (${params})`);
                            }
                        }
                        if (newlyAddedEntries.length) {
                            ...
                            await sql.query(`INSERT INTO ${tableName} (${foreignKey}, ${this.keyField}, ModifiedByUserId, CreatedByUserId) VALUES ${insertParams}`);
                        }
                    }
                }
            }
    ...
    async hardDelete({ id }) {
        ...
        for (const relatedField of relatedFields) {
            let result = await sql.query(`SELECT * FROM ${relatedField} WHERE ${keyField} = ${Number(id)}`);
            if (result.length) {
    ...
    async delete({ id, values = {} }) {
        ...
        for (const childTable of childTables) {
            const forgeinKey = childTable.foreignKey || keyField;
            await sql.query(`UPDATE ${childTable.tableName} SET IsDeleted = 1 WHERE ${forgeinKey} = ${id}`);
        }
    ...
    async lookupList({ scopeId }) {
        const request = this.createRequest();
        const { keyField, defaultSortOrder: sort, displayField, clientBased, lookupListStatement = '', tableName, hasActive = false } = this;
        ...
        const where = await this.createWhere({ filterDeleted: isStandard, hasActive, tableName });
    ...
    async list({ start = 0, limit = 100, sort, filter, groupBy, include, exclude, returnCount = true }) {
        ...
        for (const relation of relations) {
            ...
            const deleteStatement = this.softDelete ? "WHERE IsDeleted = 0" : "";
