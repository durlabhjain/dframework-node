...
const compareLookups = {
    ...
    "isAnyOf": function ({ v, type }) {
        ...
        if (Array.isArray(v) && v.length && v.every(n => typeof n === 'number' && Number.isFinite(n))) {
            const getDecimalSqlTypeForArray = (values) => {
                let maxScale = 0;
                ...
                for (const n of values) {
                    ...
                    // Handle scientific notation by expanding to a fixed representation.
                    if (str.includes('e') || str.includes('E')) {
                        // 15 is within JS double precision; adjust if needed for your domain.
                    ...
                    maxIntegerDigits = Math.max(maxIntegerDigits, intPart.length);
                    maxScale = Math.max(maxScale, fracPart.length);
                }

                let precision = maxIntegerDigits + maxScale;
                if (precision < 1) {
                    precision = 1;
                ...
                return mssql.Decimal(precision, maxScale);
            };

            sqlType = v.every(Number.isSafeInteger) ? mssql.Int : getDecimalSqlTypeForArray(v);
        }
...
class BusinessBase {
    ...
    /**
     * createWhere - Generates a WHERE clause object based on the current business object's configuration and optional parameters.
     * @param {Object} alias - The alias to use for table references in the WHERE clause (default is "Main"). 
     * @returns {Object} A WHERE clause object that can be used in SQL queries, including client-based filtering and soft delete conditions if applicable.
     */
    ...
    async getListStatement({ scopeId, forLookup = false, ...otherListParameters } = {}) {
    ...
    async lookupList({ scopeId }) {
        ...
        let { listStatement, isStandard } = await this.getListStatement({ scopeId, forLookup: true });
    ...
    async list({ start = 0, limit = 100, sort, filter, groupBy, include, exclude, returnCount = true, ...options }) {
        ...
        let { listStatement, isStandard } = await this.getListStatement(hookParameters);
        ...
        if (sort) {
            ...
            orderByFields = orderByFields.map(field => {
