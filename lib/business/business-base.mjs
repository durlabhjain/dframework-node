...
class BusinessBase {
    ...
    getSelectStatement(alias = 'Main') {
        ...
        const statement = this.selectStatement || `SELECT ${alias}.* FROM ${tableName} ${alias}`;
        perfEnd = performance.now();
            loggerToUse.info({ msg: `Execution time taken for getSelectStatement: ${perfEnd - perfStart} ms`, statement });
        return statement;
    }
    ...
    createRequest() {
        ...
        let perfStart = 0, perfEnd = 0;
        perfStart = performance.now();
        const request = BusinessBase.businessObject.sql.createRequest(loggerToUse);
        perfEnd = performance.now();
            loggerToUse.info({ msg: `Execution time taken for createRequest: ${perfEnd - perfStart} ms` });
        return request;
    }
    ...
    getRelationAdditionalQuery({ sql, request, relationWhere, insertFields = [], insertValues = [] }) {
        ...
        perfEnd = performance.now();
            loggerToUse.info({ msg: `Execution time taken for getRelationAdditionalQuery: ${perfEnd - perfStart} ms`, additionalQuery });
    ...
    getListStatement() {
        ...
        let perfStart = 0, perfEnd = 0;
        perfStart = performance.now();
        const statement = this.listStatement || (this.standardTable && this.useView !== false ? `SELECT * FROM vw${this.getTableName()}List Main` : this.getSelectStatement());
        perfEnd = performance.now();
            loggerToUse.info({ msg: `Execution time taken for getList: ${perfEnd - perfStart} ms`, statement });
        return statement;
    }
