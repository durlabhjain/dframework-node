...
class BusinessBase {
            ...
            try {
                if (Object.keys(multiSelectValues).length) {
                    for (const [colName, colValue] of Object.entries(multiSelectValues)) {
                        ...
                        const childRecordKeyField = multiSelectColumns[colName].keyField || `${tableName}Id`;
                        ...
                        if (newlyAddedEntries.length) {
                            ...
                            console.log(`INSERT INTO ${tableName} (${foreignKey}, ${this.keyField}, ModifiedByUserId, CreatedByUserId) VALUES ${insertParams}`);
    ...
    async lookupList({ scopeId }) {
        const request = this.createRequest();
        const { keyField, defaultSortOrder: sort, displayField, clientBased, lookupListStatement = '', tableName } = this;
