...
class BusinessBase {
     ...
     */
    async list({ start = 0, limit = 100, sort, filter, groupBy, rowGroupField, rowGroupAggregations, include, exclude, returnCount = true, ...options }) {
        ...
        query = sql.addParameters({ query, request, parameters: where, forWhere: true });
        // Snapshot before ORDER BY/paging/GROUP BY are appended below, so row grouping can
