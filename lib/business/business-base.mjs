        ...
        if (result.success) {
            ...
            if (Object.keys(multiSelectValues).length) {
                for (const [colName, colValue] of Object.entries(multiSelectValues)) {
                    ...
                    const surrogateKey = `${tableName}Id`;
                    const dataRes = await sql.query(`SELECT ${foreignKey} FROM ${tableName} where IsDeleted = 0 and ${this.keyField}=${id};`);
                    const mainDataRes = await sql.query(`SELECT * FROM ${tableName} WHERE IsDeleted = 0 and ${this.keyField}=${id};`);
                    ...
                    const removedSurrogateKeys = Array.from(mainDataRes).filter(entry => {
                        if(removedEntries.includes(entry[foreignKey])) return entry[surrogateKey];
                    }).map(item => item[surrogateKey]);
                    if (removedEntries.length) {
                        ...
                        if(useDeleteKey)
                        {
                            removedSurrogateKeys.map(async(item) => {
                                await sql.query(`UPDATE ${tableName} SET IsDeleted = 1, DeleteKey = ${item} WHERE ${this.keyField} = ${id} AND ${foreignKey} IN (${params}) AND ManufacturerAliasId = ${item}`);
                            })
                        }
                        else{
                            await sql.query(`UPDATE ${tableName} SET IsDeleted = 1 WHERE ${this.keyField} = ${id} AND ${foreignKey} IN (${params})`);
                        }
                    }
