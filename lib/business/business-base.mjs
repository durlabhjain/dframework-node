        ...
        if (result.success) {
            ...
            if (Object.keys(multiSelectValues).length) {
                for (const [colName, colValue] of Object.entries(multiSelectValues)) {
                    ...
                    const result = dataRes.reduce((acc, item) => {
                    ...
                    if (removedEntries.length) {
                        ...
                        if (useDeleteKey){
                            removedEntries.forEach(async(item) => {
                                if(result[item]){
                                    await sql.query(`UPDATE ${tableName} SET IsDeleted = 1, DeleteKey = ${result[item]} WHERE ${this.keyField} = ${id} AND ${foreignKey} IN (${params}) AND ${primaryKey} = ${result[item]}`);
                                }
