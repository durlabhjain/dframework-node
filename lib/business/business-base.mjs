...
class BusinessBase {
        ...
        if (result.success) {
            ...
            if (Object.keys(multiSelectValues).length) {
                for (const [colName, colValue] of Object.entries(multiSelectValues)) {
                    const { foreignKey, tableName, typeOfForeignKey = "number" } = multiSelectColumns[colName];
                    ...
                    if (newlyAddedEntries.length) {
                        ...
                        await sql.query(`INSERT INTO ${tableName} (${foreignKey}, ${this.keyField}) VALUES ${insertParams}`);
                    }
                }
            }
    ...
    async lookupList({ scopeId }) {
        const request = this.createRequest();
        const { keyField, defaultSortOrder: sort, displayField = 'DisplayValue', clientBased, lookupListStatement = '' } = this;
