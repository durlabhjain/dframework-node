...
class BusinessBase {
    ...
    async list({ start = 0, limit = 100, sort, filter, groupBy, include, exclude, returnCount = true }) {
        ...
        if (sort) {
            sort = sql.applyShadowColumns(sort);
            let orderByFields = sort.split(',');
            orderByFields = orderByFields.map(field => {
                const parts = field.trim().split(/\s+/);
                // Sanitize the bare field name first, then optionally wrap with a
                // case-normalising function (e.g. UPPER). The wrapping is applied
                // after sanitization so the resulting expression is trusted/safe.
                const fieldName = SqlHelper.sanitizeField(parts[0]);
                const direction = parts.slice(1).join(' ');
                const wrappedField = sql.applyOrderByCaseInsensitive(fieldName);
