...
const parseJson = function (json, defaultValue = null) {
...
const compareLookups = {
    ...
    "notContains": function ({ v }) {
        return { operator: 'NOT LIKE', value: `%${v}%` };
    },
    "equals": function ({ v }) {
        return { operator: '=', value: v === '' ? null : v };
    },
    "=": function ({ v }) {
        return { operator: '=', value: v === '' ? null : v };
    },
    ...
    "isEmpty": function ({ type, isNullable = true }) {
    ...
    "greaterThanOrEqual": function ({ v }) {
        return { operator: '>=', value: v };
    },
    ">=": function ({ v }) {
        return { operator: '>=', value: v };
    },
    "lessThanOrEqual": function ({ v }) {
        return { operator: '<=', value: v };
    },
    "<=": function ({ v }) {
        return { operator: '<=', value: v };
    ...
    "is": function ({ v, type }) {
        ...
        if (dateTypeFields.includes(type)) {
            ...
            toReturn = { operator: 'BETWEEN', value: values, sqlType: mssql.Varchar };
        } else {
            toReturn = { operator: '=', value: v };
        }
        return toReturn;
    },
    "not": function ({ v }) {
        ...
        return { operator: 'NOT BETWEEN', value: values, sqlType: mssql.Varchar };
    },
    "onOrAfter": function ({ v }) {
        return { operator: '>', value: `${v} ${enums.startDateTime}` };
    },
    "onOrBefore": function ({ v }) {
        return { operator: '<', value: `${v} ${enums.endDateTime}` };
    },
    "after": function ({ v }) {
        return { operator: '>', value: `${v} ${enums.endDateTime}` };
    },
    ...
    "isAnyOf": function ({ v }) {
        return { operator: 'IN', value: v, sqlType: mssql.VarChar };
    },
    "isTrue": function () {
    ...
    "isOnOrBefore": function ({ v }) {
        return { operator: '<=', value: v };
    },
    "isOnOrAfter": function ({ v }) {
        return { operator: '>=', value: v };
    },
    "isToday": function () {
...
const classMap = {
    ...
    register: function (name, configOrClass) {
        if (configOrClass.prototype instanceof BusinessBase) {
            ...
            const DerivedType = extendClass(BusinessBase, { standardTable: true, clientBased: true, tableName: name, keyField: `${name}Id`, ...configOrClass });
...
class BusinessBase {
    ...
    async save({ id, relations, ...values }) {
        const { relations: definedRelations = [], keyField, isStandard = true, readOnlyColumns = [], user, clientBased, columnOverrideValue, multipleSurveyAllowedValue } = this;
        ...
        if (requestValues && requestValues.FrequencyType && requestValues.FrequencyType === columnOverrideValue) {
            requestValues.MultipleSurveyAllowed = multipleSurveyAllowedValue;
        }
    ...
    async isUsedInSurvey(id) {
        const request = this.createRequest();
        const query = `SELECT COUNT(1) as count FROM SurveyMasterAssignment WHERE PrimaryKeyId = ${id}`;
        const result = await request.query(query);
        if (result?.recordset) {
            const count = result.recordset[0].count;
            return count > 0;
    ...
    async delete({ id, ...values }) {
        ...
        const isUsedInSurvey = await this.isUsedInSurvey(id);
        const isSurveyStarted = await this.isSurveyStarted(id);
        if (isUsedInSurvey === true) {
            throw new Error(`You cannot remove the ${errorMessage || this.tableName} as its used in survey`);
        }
    ...
    async list({ start = 0, limit = 100, sort, filter, groupBy, include, exclude, returnCount = true }) {
        ...
        const { keyField } = this;
        const sql = BusinessBase.businessObject.sql;
        const whereArr = parseJson(filter, []);
