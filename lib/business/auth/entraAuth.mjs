...
class EntraIdAuth {
    ...
    */
    ...
    async authenticateUser({ username }) {
        ...
        const menuData = await this.auth.getMenuData(roleId);
     ...
     * it attempts to authenticate the user by their email. Throws an error if the user is not authorised.
     *
     ...
     *
     * @throws {Error} - Throws an error if the session is expired or if the user is not authorised.
     */
    ...
    async authenticate({ req, res }) {
        ...
        if (!userData) {
            ...
            if (!userData) {
                throw new Error(`You (${email}) are not authorised to access the application. Please contact the administrator or log out of your Microsoft account and try again.`);
            }
        };
