...
class EntraIdAuth {
    ...
    */
    ...
    async entraLogin(req) {
        ...
        if (state.csrfToken === req.session.csrfToken && state.appStage === enums.ENTRA_APP_STAGES.SIGN_IN) {
            ...
            try {
                ...
                throw new Error(`Failed to acquire token by authorization code: ${error.message}`);
            }
        } else {
    ...
    async authenticate({ req }) {
        const { account, isAuthenticated } = req.session;
        if (!account || !isAuthenticated) {
            await this.entraLogin(req);
        }
        let userData = req.session.user;
        const { idTokenClaims = {} } = req.session.account ?? {}; // accessing the fresh session
