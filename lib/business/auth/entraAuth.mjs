...
class EntraIdAuth {
    ...
    */
    ...
    async authenticateUser({ email, groups }) {
        ...
        const { roleId, permissions } = await this.auth.getRoleAndPermissionsByAuthType({ email, groups });
    ...
    async authenticate({ req, res }) {
        ...
        if (!userData) {
            ...
            if (!userData) {
                throw new Error(`Your email (${email}) is not authorized to access the application. Please contact the administrator or log out of your Microsoft account and try again.`);
            }
        };
