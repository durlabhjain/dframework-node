    ...
    async entraLogin(req) {
        ...
        if (state.csrfToken === req.session.csrfToken && state.appStage === enums.ENTRA_APP_STAGES.SIGN_IN) {
            ...
            try {
                ...
                // Adding the user information to the session
                const { idTokenClaims: { preferred_username: email = '', groups } = {} } = tokenResponse;
                req.session.user  = await this.authenticateUser({ email, groups });
