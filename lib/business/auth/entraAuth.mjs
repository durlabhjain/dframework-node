...
class EntraIdAuth {
    ...
    */
    ...
    async entraLogin(req) {
        ...
        if (state.csrfToken === req.session.csrfToken && state.appStage === enums.ENTRA_APP_STAGES.SIGN_IN) {
            ...
            try {
                ...
                req.session.user = { entraAccount: tokenResponse.account };
                return { success: true };
            } catch (error) {
    ...
    async authenticate({ req }) {
        const { entraAccount } = req.session.user || {};
        if (!entraAccount) {
        ...
        const { entraAccount: { idTokenClaims = {} } } = userData ?? {}; // accessing the fresh session
