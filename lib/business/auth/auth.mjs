...
class Auth {
    async getUserFromDatabase(username) {
        ...
        let query = `SELECT su.Username, su.EmailAddress, su.RoleId, su.UserId, su.PasswordHash, su.DashboardSortModel, ru.Name AS RoleName FROM Security_User su JOIN Security_Role ru ON su.RoleId = ru.RoleId WHERE su.EmailAddress=@_username AND su.IsActive=1;`;
