...
class Auth {
    ...
    async getUserFromDatabase({ username, password }) {
        const passwordHash = User.encryptPassword(password);
        const sql = BusinessBase.businessObject.sql;
        return await sql.query(`SELECT Username, EmailAddress, RoleId, UserId FROM Security_User WHERE EmailAddress='${username}' AND PasswordHash='${passwordHash}' AND IsActive=1 AND IsDeleted = 0;`);
    }
    /**
    ...
    */
    async basicAuth({ username, password }) {
        try {
            // Fetch user details from the database (mock implementation)
