...
class Auth {
    ...
    async getUserFromDatabase({ username }) {
        ...
        return await sql.query(`SELECT su.Username, su.EmailAddress, su.RoleId, su.UserId, su.PasswordHash, su.TimeZoneId, r.Name FROM Security_User su LEFT JOIN Security_Role r ON su.RoleId = r.RoleId WHERE su.EmailAddress='${username}' AND su.IsActive=1 AND su.IsDeleted = 0;`);
    }
