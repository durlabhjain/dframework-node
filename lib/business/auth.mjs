...
class Auth {
    ...
    queries = {
        ...
        getSecurityUser: 'Security_Login'
    };
    /**
    ...
    async getMenuDetails(roleIds, permissions) {
        ...
        const filteredMenuDetails = menuDetails.filter(item => {
            const permission = permissions.find(p => p.Module === item.Module);
            return permission;
        })
    ...
    async authorize({ username, password, methodKey = consts.basicAuth, req, res, next, ...props }) {
        ...
        try {
            ...
            if (!permissions || !menuDetails) {
                ...
                if (!menuDetails) {
                    menuDetails = await this.getMenuDetails(roles, permissions);
                }
            }
