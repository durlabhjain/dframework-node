    ...
    async getRoleAndPermissionsByAuthType({ email, groups }) {
        ...
        switch (authType.toUpperCase()) {
            case authTypes.db:
                ...
                roleId = user.RoleId;
                permissions = await this.getRolesAndPermissions({ user });
                break;

            case authTypes.auth:
                const matchingRoles = await this.getRolesAndPermissions({ groups });
                roleId = matchingRoles.roleId;
                permissions = matchingRoles.permissions;
                break;

            case authTypes.mapping:
                const roleLookup = config.roleMapping || {};
                const roles = groups.map(g => roleLookup[g]).filter(Boolean);
                const mappedRoles = await this.getRolesAndPermissions({
                    roles
                });
                roleId = mappedRoles.roleId;
