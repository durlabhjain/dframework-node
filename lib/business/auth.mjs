...
class Auth {
    ...
    async getUserFromDatabase(username) {
        const { request } = this;
        let query = this.queries.getUser;
        query = this.sql.addParameters({ query: query, request, parameters: { _username: username }, forWhere: false });
    ...
    async getMenuData(roleId) {
        const { request } = this;
        let query = this.queries.getMenu;
        query = this.sql.addParameters({ query: query, request, parameters: { _roleId: roleId }, forWhere: false });
