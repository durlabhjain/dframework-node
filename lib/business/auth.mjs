...
class Auth {
    ...
    async getUserFromDatabase({ username }) {
        ...
        return await sql.query(`SELECT su.Username, su.EmailAddress, su.RoleId, su.UserId, su.PasswordHash FROM Security_User su WHERE su.EmailAddress='${username}' AND su.IsActive=1 AND su.IsDeleted = 0;`);
    }
    ...
    */
    async basicAuth({ username, password }) {
        try {
            ...
            const isPasswordValid = hashedPassword === user.PasswordHash;
            if (!isPasswordValid) {
    ...
    */
    ...
    async authorise({ username, password, methodKey = "BASIC" }) {
        if (username === "johndoe") {
            return {
                username: "johndoe",
        ...
        // Determine the method based on flags
        ...
        const user = await authMethod.call(this, { username, password });
        if (user) {
            return {
                userDetails: {
