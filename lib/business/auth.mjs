...
class Auth {
    ...
    // comapre password hash in sql only, getting timezone from sql.
    async getUserFromDatabase({ username }) {
        const sql = BusinessBase.businessObject.sql;
        return await sql.query(`SELECT su.Username, su.EmailAddress, su.RoleId, su.UserId, su.PasswordHash, su.TimeZoneId, tz.TimeZone FROM Security_User su LEFT JOIN TimeZone tz ON su.TimeZoneId = tz.TimeZoneId WHERE su.EmailAddress='${username}' AND su.IsActive=1 AND su.IsDeleted = 0;`);
    }
