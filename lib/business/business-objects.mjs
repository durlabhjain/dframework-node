...
const tableLookupTypes = { ClientUserLookup: { type: "ClientUserLookup", typeId: enums.LookupType.ClientUserLookup }, AlertUserLookup: { type: "AlertUserLookup", typeId: enums.LookupType.AlertUserLookup } };
...
class BusinessBaseObjectsRouter {
  ...
  setupRoutes() {
    ...
    this.router.get('/:businessObjectName/lookups', queryBase(async (req) => {
      ...
      return {
        success: true,
        data: await this.getLookups({ lookups, user: { ...businessObject.user, scopeId }, tableLookupFields: businessObject.tableLookupFields })
      };
    }));
