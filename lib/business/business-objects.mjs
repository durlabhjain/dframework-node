  ...
  setupRoutes() {
    ...
    this.router.post('/:businessObjectName/list', queryBase(async (req, res) => {
      ...
      return {
        ...
        lookups: await this.getLookups({ lookups, user: businessObject.user, tableLookupFields: BusinessBase.tableLookupFields }),
  ...
  async getLookups({ lookups, user = {}, tableLookupFields = {} }) {
    ...
    if (lookupTypes.length) {
      ...
      for (const lookupType of lookupTypes) {
        ...
        if (tableLookupFields[nameOrId]) {
          const { typeId } = tableLookupFields[nameOrId];
