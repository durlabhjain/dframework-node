...
class BusinessBaseObjectsRouter {
  ...
  setupRoutes() {
    ...
    this.router.post('/:businessObjectName/list', queryBase(async (req, res) => {
      ...
      const { start, limit, sort, groupBy, include, exclude, where, filename, columns, lookups } = req.body;

      const data = await businessObject.list({ start, limit, sort, filter: where, groupBy, include, exclude, columns });

      if (filename) {
        res.attachment(filename);
      }

      return { success: true, ...data, exportColumns: JSON.parse(columns || '[]'), userDateTimeFormat: businessObject.user.tags?.DateTimeFormat?.toUpperCase(), userDateFormat: businessObject.user.tags?.DateFormat?.toUpperCase(), userCurrencySymbol: businessObject.user.tags?.CurrencySymbol, lookups: await this.getLookups({ lookups, user: businessObject.user }) };
    }, { data: 'records' }));
    ...
    this.router.get('/:businessObjectName/:id', queryBase(async (req) => {
      ...
      return { success: true, data, lookups: await this.getLookups({ lookups, user: businessObject.user }) };
    }));
