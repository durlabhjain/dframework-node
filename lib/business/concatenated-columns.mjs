        ...
        for (const colConfig of columns) {
            ...
            if (!keys[parentKeyColumn]) {
                ...
                for (const dr of records) {
                    const value = dr[parentKeyColumn];
                    if (value !== null && value !== undefined) {
                        idSet.add(value);
                    }
                }
                keys[parentKeyColumn] = [...idSet];
            }
            ...
            if (ids.length > 0) {
                ...
                if (colConfig.listMethod) {
                    ...
                    while (startIndex < ids.length) {
                        ...
                        const { success, err, data: tempDt } = await sql.execute({ query, request });
                        if (!success) {
                            throw err;
                        }
                        results = results.concat(Array.isArray(tempDt) ? tempDt : []);
                    }
                }
                ...
                records.forEach(dr => {
                    if (dr[parentKeyColumn] !== null && dr[parentKeyColumn] !== undefined) {
