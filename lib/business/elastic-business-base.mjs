...
class ElasticBusinessBase extends BusinessBase {
    ...
    async response({ query, indexName, useScroll = false, isElastic, isPast }) {
        useScroll = isPast || useScroll;
        console.log("isPast", isPast);
        console.log("isElastic", useScroll);
        const records = [];
    ...
    async getRecords({ query, responseType, filter, sort, isElastic, isPast }) {
        const { relations, indexName } = this;

        const { records, totalRecords, aggregations } = await this.response({ query, responseType, indexName, filter, sort, isElastic, isPast });
    ...
    async fetch({ start = 0, limit = 10, sort, filter, include, returnCount = true, responseType, isElastic }) {
        ...
        let isPast = limit > 1000;
        const response = await this.getRecords({ query: baseQuery, responseType, filter, sort, isElastic, isPast });
