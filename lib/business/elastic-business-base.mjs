...
class ElasticBusinessBase extends BusinessBase {
    ...
    async fetch({ start = 0, limit = 10, sort, filter, include, returnCount = true, responseType, isElastic }) {
        ...
        if (start >= 10000) {
            ...
            const response = await this.response({ query: scrollQuery, responseType, indexName: this.indexName, useScroll: true, isElastic });
            const { aggregations, records } = response;
            const filteredRecords = records.slice(start % 10000, (start % 10000) + limit);
    
            if (returnCount) {
                const recordCount = response.totalRecords;
                return { records: filteredRecords, recordCount, aggregations }
            } else {
                return { records: filteredRecords, aggregations }
            }
        } else {
