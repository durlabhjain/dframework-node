...
class ElasticBusinessBase extends BusinessBase {
    ...
    async scrollResponse(scrollId) {
        return await BusinessBase.businessObject.elastic.requestAdapter.getJson({
    ...
    async response({ query, indexName, useScroll = false, isElastic, start = 0, limit = 10 }) {
        ...
        const records = [];
        let url = `${BusinessBase.businessObject.elastic.baseUrl}/${indexName}/_search${useScroll ? '?scroll=1m' : ''}`, totalRecords, aggs = {}, scrollId, currentHits = [];
            ...
            while (currentHits.length > 0) {
                if (!isElastic) {
                    ...
                    while (fetchedRecords + currentHits.length <= start) {
                        ...
                        const scrollResponse = await this.scrollResponse(scrollId);
