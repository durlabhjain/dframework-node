    ...
    async response({ query, indexName, useScroll = false, isElastic }) {
        ...
        do {
            ...
            if (!useScroll || !partialRecords) {
                ...
                if (endIndex >= from) {
                    const toRemoveFromEnd = endIndex - limit;
                    if (toRemoveFromEnd > 0) {
                        currentHits.splice(currentHits.length - toRemoveFromEnd, toRemoveFromEnd);
                    }
                    const toRemove = from - startIndex;
                    if (toRemove > 0) {
                        currentHits.splice(0, toRemove);
                    }
                    records.push(...currentHits);
                }
            }
