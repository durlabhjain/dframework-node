...
class ElasticBusinessBase extends BusinessBase {
    ...
    async response({ query, indexName, useScroll = false, isElastic, start = 0, limit = 10 }) {
        useScroll = isElastic || useScroll;
        const records = [];
        let url = `${BusinessBase.businessObject.elastic.baseUrl}/${indexName}/_search${useScroll ? '?scroll=1m' : ''}`,
            totalRecords, aggs = {},
            scrollId, currentHits = [];

        if (useScroll) {
            query.size = isElastic ? 10000 : Math.min(start + limit, 10000);
            delete query.from;
            ...
            while (currentHits.length > 0) {
                if (!isElastic) {
                    // Skip records if not exporting and starting from a specific position
                    let fetchedRecords = 0;
                    while (fetchedRecords + currentHits.length <= start) {
                        fetchedRecords += currentHits.length;
                        const scrollResponse = await BusinessBase.businessObject.elastic.requestAdapter.getJson({
                    ...
                    // Collect required records after skipping
                    const startIndex = start - fetchedRecords;
                    records.push(...currentHits.slice(startIndex, startIndex + limit));
                    break;
                } else {
            ...
            // Clear scroll context after all data is fetched
            ...
            // Handle non-scroll requests
            const { hits, aggregations = {} } = await BusinessBase.businessObject.elastic.requestAdapter.getJson({
                url,
                body: query
            });
    ...
    async getRecords({ query, responseType, filter, sort, isElastic, start, limit, useScroll }) {
        const { relations, indexName } = this;

        const { records, totalRecords, aggregations } = await this.response({ query, responseType, indexName, filter, sort, isElastic, start, limit, useScroll });
    ...
    async fetch({ start = 0, limit = 10, sort, filter, include, returnCount = true, responseType, isElastic }) {
        ...
        const response = await this.getRecords({ query: baseQuery, responseType, filter, sort, isElastic, start, limit, useScroll: start >= 10000 });
