...
class ElasticBusinessBase extends BusinessBase {
    ...
    async getRecords({ query, responseType, filter, sort, isElastic, useScroll }) {
        const { relations, indexName } = this;

        const { records, totalRecords, aggregations } = await this.response({ query, responseType, indexName, filter, sort, isElastic, useScroll });
    ...
    async fetch({ start = 0, limit = 10, sort, filter, include, returnCount = true, responseType, isElastic }) {
        ...
        let useScroll = limit > 1000;
        const response = await this.getRecords({ query: baseQuery, responseType, filter, sort, isElastic, useScroll });
