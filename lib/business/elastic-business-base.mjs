...
class ElasticBusinessBase extends BusinessBase {
    ...
    async getBaseQuery() {
        if (this.baseQuery !== null) {
            if (this.baseQuery.endsWith(".esquery")) {
                // Resolve the correct path based on the current script's directory
                const filePath = path.resolve(process.cwd(), "queries", this.baseQuery);
                this.baseQuery = (await fs.readJson(filePath));
            }
        }
        else {
            this.baseQuery = util.elasticBaseQuery;
        }
