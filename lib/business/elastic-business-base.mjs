...
class ElasticBusinessBase extends BusinessBase {
    ...
    async response({ query, indexName, useScroll = false, isElastic }) {
        ...
        do {
            ...
            if (partialRecords && (from + size) > this.elasticLimit) {
                const hitsToAdd = hits.hits.slice(Math.max(0, from - recordsFetched), Math.max(0, from - recordsFetched + size - records.length));
                records.push(...hitsToAdd);
                recordsFetched += hits.hits.length;
            } else {
                records.push(...hits.hits);
            }

        } while (useScroll && currentHits?.length && records.length < limit);
