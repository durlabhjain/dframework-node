...
class ElasticBusinessBase extends BusinessBase {
    ...
    async response({ query, indexName, useScroll = false, isElastic, start = 0, limit = 10 }) {
        ...
        if (useScroll) {
            ...
            while (currentHits.length > 0 && currentCount < totalRecords) {
                for (let hit of currentHits) {
                    if (isElastic || (currentCount >= start && records.length < limit)) {
                        records.push(hit);
                    }
                    currentCount++;
                    if (!isElastic && (records.length >= limit || currentCount >= totalRecords)) {
                        break;
                    }
                }
                if (!isElastic && (records.length >= limit || currentCount >= totalRecords)) {
            ...
            //Delete scrolling after finishing fetching 
    ...
    async fetch({ start = 0, limit = 10, sort, filter, include, returnCount = true, responseType, isElastic }) {
        ...
        const response = await this.getRecords({ query: baseQuery, responseType, filter, sort, isElastic, start, limit, useScroll: start + limit > 10000 });
