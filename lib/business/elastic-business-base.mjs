...
class ElasticBusinessBase extends BusinessBase {
    ...
    async response({ query, indexName, useScroll = false, useSearchAfter = false, sortField = '_id', sortOrder = 'asc' }) {
        useScroll = useScroll || useSearchAfter;
        const records = [];
        let url = `${BusinessBase.businessObject.elastic.baseUrl}/${indexName}/_search${useScroll ? '?scroll=1m' : ''}`, currentHits, totalRecords, aggs = {};
        let lastHitSort = null;

        do {
            if (useSearchAfter && lastHitSort) {
            ...
            if (useSearchAfter && currentHits?.length) {
                lastHitSort = currentHits[currentHits.length - 1].sort;
            }
        } while (currentHits?.length && (!useSearchAfter || lastHitSort));
    ...
    async getRecords({ query, responseType, filter, sort, isElastic, useSearchAfter, sortField, sortOrder }) {
        const { relations, indexName } = this;

        const { records, totalRecords, aggregations } = await this.response({ query, responseType, indexName, filter, sort, isElastic, useSearchAfter, sortField, sortOrder });
    ...
    async fetch({ start = 0, limit = 10, sort, filter, include, returnCount = true, responseType, isElastic }) {
        ...
        let sortField = '_id';
        let sortOrder = 'asc';
        if (start >= 9000) { // Threshold to switch to search_after
            useSearchAfter = true;
            baseQuery.size = limit;
            sortField = sort ? sort.split(',')[0].split(' ')[0] : '_id';
            sortOrder = sort ? sort.split(',')[0].split(' ')[1] || 'asc' : 'asc';
        } else {
        ...
        const response = await this.getRecords({
            query: baseQuery, responseType, filter, sort, isElastic,
            useSearchAfter, sortField, sortOrder
        });
